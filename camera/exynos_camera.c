 ...
 * Copyright (C) 2013 Daniel Hillenbrand <codeworkx@cyanogenmod.org>
 *
...
struct exynos_camera_preset exynos_camera_presets_smdk4412[] = {
	{
		.name = "S5C73M3",
		...
		.params = {
			...
			.preview_frame_rate_values = "30,20,15,10",
			.preview_frame_rate = 30,
			.preview_fps_range_values = "(10000,10000),(15000,15000),(15000,30000),(30000,30000)",
			.preview_fps_range = "15000,30000",

			.picture_size_values = "640x480,960x720,1024x768,1280x720,1600x1200,2560x1920,3264x2448,2048x1536,3264x1836,2048x1152,3264x2176",
			.picture_size = "3264x2176",
			.picture_format_values = "jpeg",
			.picture_format = "jpeg",
			.jpeg_thumbnail_size_values = "160x120,160x90,144x96,0x0",
			...
			.recording_size_values = "1920x1080,1280x720,960x720,720x480,640x480,352x288,320x240,176x144",
			...
			.effect = "none",
			.effect_values = "none,mono,negative,sepia,solarize,posterize,washed,vintage-warm,vintage-cold,point-blue,point-red-yellow,point-green,cartoonize",
	...
	},
	{
        ...
        .hybrid_mode = 0,
		.params = {
			.preview_size_values = "960x720,1280x720,704x704,352x288,640x480,320x320,320x240",
			.preview_size = "960x720",
			.preview_format_values = "yuv420sp,yuv420p",
			.preview_format = "yuv420sp",
			.preview_frame_rate_values = "30,20,15,10,8",
			.preview_frame_rate = 15,
			.preview_fps_range_values = "(8000,8000),(10000,10000),(15000,15000),(15000,30000),(30000,30000)",
			.preview_fps_range = "8000,30000",

			.picture_size_values = "1280x960,1392x1392,640x480,1280x720,720x480,320x240",
			.picture_size = "1280x960",
			.picture_format_values = "jpeg",
			.picture_format = "jpeg",
			.jpeg_thumbnail_size_values = "160x120,160x160,160x90,144x96,0x0",
			.jpeg_thumbnail_width = 160,
			.jpeg_thumbnail_height = 120,
			.jpeg_thumbnail_quality = 100,
			.jpeg_quality = 90,

			.recording_size = "1280x720,960x720,720x480,640x480,352x288,320x320,320x240,176x144",
			.recording_size_values = "1280x720",
			.recording_format = "yuv420sp",
...
struct exynos_v4l2_node exynos_v4l2_nodes_smdk4412[] = {
	{
		.id = 0,
		.node = "/dev/video0",
	},
...
struct exynox_camera_config exynos_camera_config_smdk4412 = {
	.presets = (struct exynos_camera_preset *) &exynos_camera_presets_smdk4412,
	.presets_count = 2,
	.v4l2_nodes = (struct exynos_v4l2_node *) &exynos_v4l2_nodes_smdk4412,
	.v4l2_nodes_count = 3,
};
...
int exynos_camera_params_init(struct exynos_camera *exynos_camera, int id)
{
    ...
    exynos_camera->camera_hybrid_mode = exynos_camera->config->presets[id].hybrid_mode;
...
int exynos_camera_params_apply(struct exynos_camera *exynos_camera)
{
	...
	if (preview_format_string != NULL) {
        ...
        } else if (strcmp(preview_format_string, "jpeg") == 0) {
            preview_format = V4L2_PIX_FMT_JPEG;
            preview_format_bpp = 0.0f;
		} else {
    ...
    // FIXME
	jpeg_resolution = 0x4000300;
	if (jpeg_resolution > 0) {
		exynos_camera->jpeg_resolution = jpeg_resolution;
		rc = exynos_v4l2_s_ctrl(exynos_camera, 0, V4L2_CID_CAMERA_JPEG_RESOLUTION, jpeg_resolution);
		if (rc < 0)
    ...
    // Hybrid mode
    if (exynos_camera->camera_hybrid_mode == 1)
        hybrid_mode = 1;
    else
        hybrid_mode = 0;

    ALOGD("%s: Hybrid mode = %d", __func__, hybrid_mode);
    rc = exynos_v4l2_s_ctrl(exynos_camera, 0, V4L2_CID_CAMERA_HYBRID, hybrid_mode);
    if (rc < 0)
        ALOGE("%s: s ctrl failed!", __func__);
...
int exynos_camera_picture(struct exynos_camera *exynos_camera)
{
    ...
    {
        ALOGD("%s: callback(CAMERA_MSG_SHUTTER)", __func__);
        exynos_camera->callbacks.notify(CAMERA_MSG_SHUTTER, 0, 0, exynos_camera->callbacks.user);
    }

    // FIXME: I'm not sure about this
	if (camera_picture_format == V4L2_PIX_FMT_JPEG && exynos_camera->picture_memory->data != NULL && exynos_camera->picture_buffer_length >= 0) {
        if (exynos_camera->callbacks.request_memory != NULL) {
            picture_data_memory = exynos_camera->callbacks.request_memory(-1, exynos_camera->picture_buffer_length, 1, 0);
        ...
        memcpy(picture_data_memory->data, exynos_camera->picture_memory->data, exynos_camera->picture_buffer_length);
    }

	if (EXYNOS_CAMERA_MSG_ENABLED(CAMERA_MSG_RAW_IMAGE) && EXYNOS_CAMERA_CALLBACK_DEFINED(data) && picture_data_memory != NULL)
    {
        ALOGD("%s: callback(CAMERA_MSG_RAW_IMAGE)", __func__);
		exynos_camera->callbacks.data(CAMERA_MSG_RAW_IMAGE, picture_data_memory, 0, NULL, exynos_camera->callbacks.user);
    }

    // FIXME
	if (EXYNOS_CAMERA_MSG_ENABLED(CAMERA_MSG_COMPRESSED_IMAGE) && EXYNOS_CAMERA_CALLBACK_DEFINED(data) && picture_data_memory != NULL)
    {
        ALOGD("%s: callback(CAMERA_MSG_COMPRESSED_IMAGE)", __func__);

/*
        // JPEG PROCESSING
        jpeg_fd = jpeghal_enc_init();
        if (jpeg_fd < 0) {
            ALOGE("%s: Failed to init JPEG hal", __func__);
            goto error;
        }

        // FIXME
        // The jpeg hal doesn't accept V4L2_PIX_FMT_JPEG
        // but if we set anything else v4l2 polling fails
        camera_picture_format = V4L2_PIX_FMT_YUV420;
        ...
        switch (camera_picture_format) {
            ...
            case V4L2_PIX_FMT_YUV420:
                jpeg_out_format = V4L2_PIX_FMT_JPEG_420;
                break;
            case V4L2_PIX_FMT_YUYV:
            case V4L2_PIX_FMT_UYVY:
            case V4L2_PIX_FMT_YUV422P:
            default:
                jpeg_out_format = V4L2_PIX_FMT_JPEG_422;
                break;
        }

        enc_config.mode = JPEG_ENCODE;
        enc_config.width = picture_width;
        enc_config.height = picture_height;
        enc_config.num_planes = 1;
        enc_config.pix.enc_fmt.in_fmt = camera_picture_format;
        enc_config.pix.enc_fmt.out_fmt = jpeg_out_format;
        ...
        jpeghal_s_ctrl(jpeg_fd, V4L2_CID_CACHEABLE, 3);

        struct jpeg_buf jpeg_in_buf;
        jpeg_in_buf.start[0] = (void *)exynos_v4l2_s_ctrl(exynos_camera, 0, V4L2_CID_PADDR_Y, index);
        jpeg_in_buf.length[0] = exynos_camera->picture_buffer_length; // ???
        jpeg_in_buf.memory = V4L2_MEMORY_USERPTR;
        jpeg_in_buf.num_planes = 1;

        struct jpeg_buf jpeg_out_buf;
        jpeg_out_buf.memory = V4L2_MEMORY_USERPTR;
        jpeg_out_buf.num_planes = 1;

        ALOGE("%s: calling jpeghal_set_inbuf()", __func__);
        if (jpeghal_set_inbuf(jpeg_fd, &jpeg_in_buf) < 0) {
            ALOGE("%s: Failed to get JPEG in buffer", __func__);
            jpeghal_deinit(jpeg_fd, &jpeg_in_buf, &jpeg_out_buf);
            goto error;
        }

        for (i = 0; i < jpeg_in_buf.num_planes; i++) {
            if ((unsigned int)jpeg_in_buf.start[i] & (SIZE_4K - 1)) {
                ALOGE("%s: JPEG start address should be aligned to 4 Kbytes", __func__);
                return -1;
            } else if ((unsigned int)enc_config.width & (16 - 1)) {
                ALOGE("%s: Image width should be multiple of 16", __func__);
                return -1;
            }
        }

        ALOGE("%s: calling jpeghal_set_outbuf()", __func__);
        if (jpeghal_set_outbuf(jpeg_fd, &jpeg_out_buf) < 0) {
            ALOGE("%s: Failed to get JPEG out buffer", __func__);
            jpeghal_deinit(jpeg_fd, &jpeg_in_buf, &jpeg_out_buf);
            goto error;
        }

        ALOGE("%s: calling jpeghal_enc_exe()", __func__);
        jpeg_result = jpeghal_enc_exe(jpeg_fd, &jpeg_in_buf, &jpeg_out_buf);
        ...
        ALOGE("%s: V4L2_CID_CAM_JPEG_ENCODEDSIZE", __func__);
        rc = jpeghal_g_ctrl(jpeg_fd, V4L2_CID_CAM_JPEG_ENCODEDSIZE);
        if (rc < 0) {
            ALOGE("%s: Failed to get JPEG out size", __func__);
            jpeghal_deinit(jpeg_fd, &jpeg_in_buf, &jpeg_out_buf);
            goto error;
        } else {
            *jpeg_out_size = (unsigned int)rc;
        }

        if (exynos_camera->callbacks.request_memory != NULL) {
            data_memory = exynos_camera->callbacks.request_memory(-1, *jpeg_out_size, 1, 0);
            if (data_memory == NULL) {
                ALOGE("%s: picture memory request failed!", __func__);
                jpeghal_deinit(jpeg_fd, &jpeg_in_buf, &jpeg_out_buf);
                goto error;
        ...
        // Copy jpeg image of output buffer
        memcpy(data_memory->data, jpeg_out_buf.start[0], *jpeg_out_size);
        picture_size = *jpeg_out_size;

        if (jpeg_fd > 0) {
            if (jpeghal_deinit(jpeg_fd, &jpeg_in_buf, &jpeg_out_buf) < 0)
                ALOGE("%s: failed on jpeghal_deinit", __func__);
            jpeg_fd = 0;
        }
        // END JPEG PROCESSING

		exynos_camera->callbacks.data(CAMERA_MSG_COMPRESSED_IMAGE, data_memory, 0, NULL, exynos_camera->callbacks.user);
*/
        exynos_camera->callbacks.data(CAMERA_MSG_COMPRESSED_IMAGE, picture_data_memory, 0, NULL, exynos_camera->callbacks.user);
    }
...
int exynos_camera_picture_start(struct exynos_camera *exynos_camera)
{
    ...
    ALOGE("%s: camera_format=%d", __func__, camera_format);
    ...
    rc = exynos_v4l2_s_fmt_priv(exynos_camera, 0, width, height, format, (enum v4l2_field) IS_MODE_PREVIEW_STILL);
    if (rc < 0) {
        ALOGE("%s: exynos_v4l2_s_fmt_priv failed!", __func__);
        return -1;
    }
	...
	rc = exynos_v4l2_g_ctrl(exynos_camera, 0, V4L2_CID_CAMERA_CAPTURE, 0);
	if (rc < 0) {
...
int exynos_camera_preview(struct exynos_camera *exynos_camera)
{
	...
	exynos_camera->gralloc->lock(exynos_camera->gralloc, *buffer, GRALLOC_USAGE_SW_WRITE_OFTEN | GRALLOC_USAGE_HW_ION,
...
int exynos_camera_preview_start(struct exynos_camera *exynos_camera)
{
    ...
    rc = exynos_v4l2_s_fmt_priv(exynos_camera, 0, width, height, format, field);
    if (rc < 0) {
        ALOGE("%s: exynos_v4l2_s_fmt_priv failed!", __func__);
        return -1;
    }
	...
	struct preview_stream_ops *w)
{
	...
	rc = w->set_usage(w, GRALLOC_USAGE_SW_WRITE_OFTEN | GRALLOC_USAGE_HW_ION);
	if (rc) {
