...
int exynos_camera_picture(struct exynos_camera *exynos_camera)
{
	...
	if (EXYNOS_CAMERA_MSG_ENABLED(CAMERA_MSG_RAW_IMAGE) && EXYNOS_CAMERA_CALLBACK_DEFINED(data) && exynos_camera->picture_memory != NULL)
    {
        ALOGD("%s: callback(CAMERA_MSG_RAW_IMAGE)", __func__);
		exynos_camera->callbacks.data(CAMERA_MSG_RAW_IMAGE, exynos_camera->picture_memory, 0, NULL, exynos_camera->callbacks.user);
    }
    ...
    // FIXME
        ...
        // Copy raw image to jpeg input buffer
        ALOGE("%s: copy raw image", __func__);
        // >>> crashes here <<<
        memcpy(jpeg_in_buf.start[0], exynos_camera->picture_memory->data, jpeg_in_buf.length[0]); // ???
        ...
        if (exynos_camera->callbacks.request_memory != NULL) {
            picture_data_memory = exynos_camera->callbacks.request_memory(-1, *jpeg_out_size, 1, 0);
        ...
        memcpy(picture_data_memory->data, jpeg_out_buf.start[0], *jpeg_out_size);
