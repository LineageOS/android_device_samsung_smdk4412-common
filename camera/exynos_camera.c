...
int exynos_camera_picture(struct exynos_camera *exynos_camera)
{
	...
	if (EXYNOS_CAMERA_MSG_ENABLED(CAMERA_MSG_RAW_IMAGE) && EXYNOS_CAMERA_CALLBACK_DEFINED(data) && data_memory != NULL)
    {
        ALOGD("%s: callback(CAMERA_MSG_RAW_IMAGE)", __func__);
		exynos_camera->callbacks.data(CAMERA_MSG_RAW_IMAGE, data_memory, 0, NULL, exynos_camera->callbacks.user);
    }
    ...
    // FIXME
        ...
        jpeghal_s_ctrl(jpeg_fd, V4L2_CID_CACHEABLE, 1);
        ...
        memcpy(jpeg_in_buf.start[0], data_memory->data, jpeg_in_buf.length[0]);
