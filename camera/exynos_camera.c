...
int exynos_camera_picture(struct exynos_camera *exynos_camera)
{
    ...
    if (exynos_camera->camera_hybrid_mode == 1)
    {
        ALOGD("%s: HYBRID MODE", __func__);
        goto error;
    }
    else
    {
        ALOGD("%s: INTERLEAVE MODE", __func__);

        rc = exynos_v4l2_g_ctrl(exynos_camera, 0, V4L2_CID_CAM_JPEG_MEMSIZE, &data_size);
		if (rc < 0) {
			ALOGE("%s: g ctrl failed!", __func__);
            return -1;
		}

        ALOGE("%s: data_size=%d", __func__, data_size);

        if (exynos_camera->callbacks.request_memory != NULL) {
            picture_data_memory = exynos_camera->callbacks.request_memory(-1, data_size, 1, 0);
        ...
        ALOGE("%s: MEMCOPY PICTURE", __func__);
        memcpy(picture_data_memory->data, exynos_camera->picture_memory->data, data_size);
    }
