...
int exynos_camera_picture(struct exynos_camera *exynos_camera)
{
	...
	if (camera_picture_format == V4L2_PIX_FMT_JPEG && jpeg_thumbnail_addr != NULL && jpeg_thumbnail_size >= 0) {
		...
		enc_config.width = jpeg_thumbnail_width;
		enc_config.height = jpeg_thumbnail_height;
        ...
        jpeg_in_buf.memory = V4L2_MEMORY_MMAP;
        jpeg_in_buf.num_planes = 1;

        struct jpeg_buf jpeg_out_buf;
        jpeg_out_buf.memory = V4L2_MEMORY_MMAP;
        jpeg_out_buf.num_planes = 1;
        
        jpeg_in_buffer = jpeghal_set_inbuf(jpeg_fd, &jpeg_in_buf);
		...
		jpeg_out_buffer = jpeghal_set_outbuf(jpeg_fd, &jpeg_out_buf);
		...
		jpeg_out_size = jpeghal_g_ctrl(jpeg_fd, V4L2_CID_CAM_JPEG_ENCODEDSIZE);
		if (jpeg_out_size <= 0) {
...
void *exynos_camera_auto_focus_thread(void *data)
{
	...
	while (exynos_camera->auto_focus_enabled == 1) {
		...
		} else if (auto_focus_status == CAMERA_AF_STATUS_SUCCESS || auto_focus_status == CAMERA_AF_STATUS_1ST_SUCCESS) { // Success
			auto_focus_result = 1;
			pthread_mutex_unlock(&exynos_camera->auto_focus_mutex);
			goto thread_exit;
		} else {
