...
SecCamera::~SecCamera()
{
    ...
    if (!_isInited) {
        ALOGV("%s: Can't destroy! SecCamera not inited!", __func__);
        return;
    }

    ALOGE_IF(this->stopPreview() < 0,
...
int SecCamera::startPreview(void)
{
    ...
    ret = _v4l2Cam->setScenarioMode(_previewWidth, _previewHeight, _previewPixfmt, (enum v4l2_field) IS_MODE_PREVIEW_STILL);
    CHECK_EQ(ret, 0);
...
int SecCamera::startRecord(void)
{
    ...
    if (_v4l2Rec == NULL) {
        ...
        if (_v4l2Rec->getFd() == 0) {
            ALOGE("failed to open rec video ch");
            return -1;
        }
    }
    ...
    ret = _v4l2Cam->setScenarioMode(_previewWidth, _previewHeight, V4L2_PIX_FMT_NV12, (enum v4l2_field) IS_MODE_CAPTURE_VIDEO);
    CHECK_EQ(ret, 0);
...
{
    if (!_isRecordOn || _v4l2Rec == NULL) {
        ALOGW("Recording stoped! will ignore dqRecordBuffer!");
        return -1;
    }
...
{
    if (!_isRecordOn || _v4l2Rec == NULL) {
        ALOGW("Recording stoped! will ignore qRecordBuffer!");
        return;
    }
...
void SecCamera::qPreviewBuffer(int index)
{
    ...
    ALOGE_IF(ret, "Failed to queue preview buffer, %d to camera!", index);
}
...
int SecCamera::setPreviewFormat(int width, int height, const char* strPixfmt)
{
    ...
    ALOGI("previewFormat=%dx%d(%s), frameSize=%d",
...
int SecCamera::startSnapshot(size_t* captureSize)
{
    ...
    ret = _v4l2Cam->setScenarioMode(_snapshotWidth, _snapshotHeight, _snapshotPixfmt, (enum v4l2_field) IS_MODE_CAPTURE_STILL);
    CHECK_EQ(ret, 0);
...
int SecCamera::getSnapshot(int xth)
{
    ...
    while (skipFirstNFrames) {
        ALOGV("skipFrames %d", skipFirstNFrames);
        _v4l2Cam->waitFrame();
...
{
    if (buffer == NULL) {
        ALOGE("%s: got null pointer!", __func__);
        return -1;
    }
    ...
    if (size < captureSize) {
        ALOGE("%s: buffer size, %d is too small! for snapshot size, %d",
...
int SecCamera::setSnapshotFormat(int width, int height, const char* strPixfmt)
{
    ...
    ALOGI("snapshotFormat=%dx%d(%s)", width, height, strPixfmt);
...
int SecCamera::startAutoFocus(void)
{
    ...
    ALOGE_IF(0 > ret, "%s:Failed to start auto focus!! ret=%d",
             __func__, ret);
...
int SecCamera::abortAutoFocus(void)
{
    ...
    ALOGE_IF(0 > ret, "%s:Failed to abort auto focus!! ret=%d",
             __func__, ret);
...
#define AF_SUCCESS 0x02
    ...
    ALOGI("AF was %s", afSuccess ? "successful" : "fail");

    if (_v4l2Cam->setCtrl(V4L2_CID_CAMERA_FINISH_AUTO_FOCUS, 0) < 0) {
        ALOGE("%s: Fail on V4L2_CID_CAMERA_FINISH_AUTO_FOCUS", __func__);
        return false;
    }
...
int SecCamera::setRotate(int angle)
{
    ...
    if (angle >= 360 || 0 > angle)
        ALOGW("%s: input angle, %d is truncated to %d!", __func__, angle, rotate);
...
int SecCamera::setSceneMode(const char* strSceneMode)
{
    ...
    ALOGE_IF(0 > ret, "%s:Failed to set scene-mode, %s!! ret=%d",
...
int SecCamera::setWhiteBalance(const char* strWhiteBalance)
{
    ...
    ALOGE_IF(0 > ret, "%s:Failed to set white-balance, %s!! ret=%d",
...
int SecCamera::setEffect(const char* strEffect)
{
    ...
    ALOGE_IF(0 > ret, "%s:Failed to set effects, %s!! ret=%d",
...
int SecCamera::setFlashMode(const char* strFlashMode)
{
    ...
    ALOGV("setting flash-mode set to %s...", strFlashMode);
    ...
    ALOGE_IF(0 > ret, "%s:Failed to set flash-mode, %s!! ret=%d",
...
int SecCamera::setBrightness(int brightness)
{
    ...
    ALOGE_IF(0 > ret, "%s:Failed to set brightness, %d!! ret=%d",
...
int SecCamera::setFocusMode(const char* strFocusMode)
{
    ...
    ALOGE_IF(0 > ret, "%s:Failed to set focus-mode, %s!! ret=%d",
...
int SecCamera::_createThumbnail(uint8_t* rawData, int rawSize)
{
    ...
    ALOGI("making thumb(%dx%d) from picture(%dx%d)...", tW, tH, pW, pH);
    if (rawSize != pW * pH * 2) {
        ALOGE("%s: rawSize=%d, expected=%d", __func__, rawSize, pW * pH * 2);
        goto nothumbnail;
    }

    thumbRawSize = tW * tH * 2;
    ALOGV("making raw image for thumbnail(%dx%d)...", tW, tH);
    thumbRawData = new uint8_t[thumbRawSize];

    if (thumbRawData && thumbRawSize) {
        ALOGV("shrinking raw image for thumbnail...");
        int ret = _scaleDownYuv422(rawData, pW, pH, thumbRawData, tW, tH);
        if (ret != 0) {
            ALOGE("%s: somthing wrong while shrinking raw image for thumbnail!",
                  __func__);
            goto nothumbnail;
        }

        ALOGV("compressing thumbnail...");
        ...
        if (thumbJpegData && thumbJpegSize) {
            ALOGV("copying compressed thumbnail to TaggerParams...");
...
int SecCamera::compress2Jpeg(unsigned char* rawData, size_t rawSize)
{
    if (rawData == NULL || rawSize == 0) {
        ALOGE("%s: null input!", __func__);
        return 0;
    }

    if (_encoder == NULL) {
        ALOGE("%s: has no encoder!", __func__);
        return 0;
    }
    ...
    if (_tagger == NULL) {
        ALOGW("No tagger! jpeg will be served without tag");
        return jpegSize;
    }

    ALOGI("creating tagged JPEG...");
...
int SecCamera::getJpeg(unsigned char* outBuff, int buffSize)
{
    ...
    if (writtenSize > buffSize) {
        ALOGE("%s: buffer overflowed!!", __func__);
        return -1;
    }

    if (!(writtenSize > 0)) {
        ALOGE("%s: Opps, nothing copied!!", __func__);
        return -1;
    }
...
int SecCamera::_convertGPSCoord(double coord, int* deg, int* min, int* sec)
{
    ...
    if (coord == 0) {
        ALOGE("%s: Invalid GPS coordinate", __func__);
...
{
    ALOGV("%s: setting default values for _exifParams...", __func__);
    memset(&_exifParams, 0, sizeof(TaggerParams));
