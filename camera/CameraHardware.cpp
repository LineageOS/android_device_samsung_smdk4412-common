...
#define CALL_WIN(F, ...)                                        \
    if (_window) {                                              \
        if (_window->F(_window, __VA_ARGS__)) {                 \
            ALOGE("%s: Failed while run %s", __func__, #F);      \
...
{
    release();
    ALOGI("CameraHardware destroyed");
}
...
status_t CameraHardware::setPreviewWindow(preview_stream_ops* window)
{
    ...
    if (_previewState == PREVIEW_RUNNING) {
        ALOGI("Preview window changed while preview is running");
        _stopPreviewLocked();
    }

    if (_window == NULL) {
        ALOGV("%s: received NULL window!", __func__);
        return OK;
    }

    int minWinBufs = 0;
    CALL_WIN(get_min_undequeued_buffer_count, &minWinBufs);
    ALOGE_IF(minWinBufs >= MAX_CAM_BUFFERS,
                                     ...
                                     const char* strPixfmt)
{
    ...
    if (!strcmp(strPixfmt, CameraParameters::PIXEL_FORMAT_YUV420P)) {
        ...
        ALOGE("%s: Unsupported preview format, %s!", __func__, strPixfmt);
        CALL_WIN(cancel_buffer, buf);
        return UNKNOWN_ERROR;
    }
...
bool CameraHardware::_previewLoop()
{
    ...
    case PREVIEW_PENDING:
        ...
        ALOGI("%s: calling _camera->stopPreview() and waiting", __func__);
        // signal that we're stopped
        _previewStoppedCondition.signal();
        _previewStateChangedCondition.wait(_previewLock);
        ALOGI("preview state changed _previewState = %d", _previewState);
        _previewLock.unlock();
        return true;

    case PREVIEW_ABORT:
    default:
        ALOGI("Exiting preview thread...");
        _previewLock.unlock();
        return false;
    }
    _previewLock.unlock();
    ...
    if (0 > ret || 0 > index) {
        ALOGW("Is preview frame not readied?");
        return true;
    }
    ...
    if (0 > ret || 0 > index) {
        ALOGW("Is record frame not readied?");
        return true;
    }
...
status_t CameraHardware::_startPreviewLocked()
{
    ...
    if (_previewState != PREVIEW_PENDING) {
        ALOGE("%s: Seems like no preview window set yet!", __func__);
        return UNKNOWN_ERROR;
    }
    ...
    if (_previewHeap == NULL) {
        ALOGE("%s: Failed to request memory for preview!", __func__);
        _camera->stopPreview();
        return NO_MEMORY;
    }
...
status_t CameraHardware::startPreview()
{
    if (_waitPictureComplete() != NO_ERROR) {
        ALOGE("%s: Too long wait for capture finish!", __func__);
        return TIMED_OUT;
    }
    ...
    if (_previewState != PREVIEW_IDLE) {
        ALOGE("%s: Expected preview is idle but, _previewState = %d",
    ...
    if (_window == NULL) {
        ALOGV("%s: No preview window set yet. preview will be started "
...
{
    if (_previewState != PREVIEW_RUNNING) {
        ALOGV("%s: Expected preview is running but, _previewState = %d",
              __func__, _previewState);
        return;
    }
...
status_t CameraHardware::startRecording()
{
    ...
    if (_camera->startRecord() < 0) {
        ALOGE("ERR(%s):Fail on _camera->startRecord()", __func__);
        return UNKNOWN_ERROR;
    }
...
void CameraHardware::stopRecording()
{
    ...
    if (_previewState != PREVIEW_RECORDING) {
        ALOGW("%s: Expected recording is running but, _previewState = %d",
              __func__, _previewState);
        return;
    }

    if (_camera->stopRecord() < 0) {
        ALOGE("ERR(%s):Fail on _camera->stopRecord()", __func__);
        return;
    }
    _previewState = PREVIEW_RUNNING;
}
...
bool CameraHardware::_focusLoop()
{
    ...
    ALOGI("_focusState = %d", _focusState);

    switch (_focusState) {
    case FOCUS_IDLE:
        _focusStateChangedCondition.wait(_focusLock);
        ALOGV("_focusState changed to %d", _focusState);
        break;

    case FOCUS_RUN_AF:
        if (_camera->startAutoFocus() < 0) {
            ALOGE("%s: Stop FocusThread!", __func__);
            return false;
        }
...
status_t CameraHardware::autoFocus()
{
    if (_focusThread == NULL) {
        ALOGE("%s: No focusThread! ignore autoFocus!", __func__);
        return NO_ERROR;
    }
    ...
    if (_focusState != FOCUS_IDLE) {
        ALOGE("%s: Expected FOCUS_IDLE but state is %d", __func__, _focusState);
        return UNKNOWN_ERROR;
    }
...
status_t CameraHardware::cancelAutoFocus()
{
    ...
    ALOGI("AF canceling... current state is %d", _focusState);
    if (_camera->abortAutoFocus() < 0) {
        ALOGE("ERR(%s):Fail on _camera->cancelAutofocus()", __func__);
        return UNKNOWN_ERROR;
    }
    _focusState = FOCUS_IDLE;
...
bool CameraHardware::_pictureLoop()
{
    ...
    ALOGV("doing snapshot...");
    ret = _camera->startSnapshot(&rawSize);
    if (ret != 0) {
        ALOGE("%s: Failed to do snapshot!", __func__);
        ret = UNKNOWN_ERROR;
        goto out;
    }
    ...
    if (_rawHeap == NULL || _rawHeap->size != rawSize) {
        ALOGV("allocating mem (%d bytes) for raw snapshot...", rawSize);
        if (_rawHeap != NULL)
            _rawHeap->release(_rawHeap);

        _rawHeap = _cbReqMemory(-1, rawSize, 1, 0);
        if (_rawHeap == NULL) {
            ALOGE("%s: Failed to create RawHeap!", __func__);
            ret = NO_MEMORY;
            goto out;
        }
    }

    ALOGV("getting raw snapshot...");
    ...
    if (_cbData && (_msgs & CAMERA_MSG_COMPRESSED_IMAGE)) {
        ...
        if (jpegHeap == NULL) {
            ALOGE("%s: Failed to get memory for jpegJeap!", __func__);
            return false;
        }

        ALOGV("getting jpeg snapshot...");
...
{
    if (NULL == newValue) {
        ALOGV("%s: no value to compare for %s!!", __func__, key);
        return false;
    }

    const char* currValue = _parms.get(key);
    if (NULL == currValue) {
        ALOGV("_parms has no key, %s", key);
        return false;
    }

    if (0 != strcmp(newValue, currValue)) {
        ALOGI("CameraParameter, %s updated: %s->%s", key, currValue, newValue);
        return true;
    }
                                     ...
                                     const char* key, int newValue) const
{
    if (NULL == _parms.get(key)) {
        ALOGV("%s: _parms has no key, %s", __func__, key);
        return false;
    }
    ...
    if (currValue != newValue) {
        ALOGI("CameraParameter, %s updated: %d->%d", key, currValue, newValue);
        return true;
    }
...
status_t CameraHardware::setParameters(const CameraParameters& parms, bool needInit)
{
    ...
    if (needInit
        ...
        ALOGV("setting preview format to %dx%d(%s)...", width, height, strPixfmt);
    ...
    if (needInit
        ...
        ALOGV("#### setting picture format to %dx%d(%s)...", width, height, strPixfmt);
    ...
    if (_isParamUpdated(parms, CameraParameters::KEY_JPEG_THUMBNAIL_WIDTH, width)
        ...
        ALOGV("setting thumbnail size to %dx%d", width, height);
    ...
    if (_isParamUpdated(parms, strKey, quality)) {
        ALOGV("setting %s to %d...", strKey, quality);
    ...
    if (_isParamUpdated(parms, strKey, rot)) {
        ALOGV("setting %s to %d...", strKey, rot);
    ...
    if (_isParamUpdated(parms, strKey, strFlashMode)) {
        ALOGV("Setting flash-mode to %s...", strFlashMode);
    ...
    if (_isParamUpdated(parms, strKey, strSceneMode)) {
        ALOGV("setting scene-mode to %s...", strSceneMode);
    ...
    if (_isParamUpdated(parms, strKey, strWhiteBalance)) {
        ALOGV("setting whitebalance to %s...", strWhiteBalance);
    ...
    if (_isParamUpdated(parms, strKey, strEffect)) {
        ALOGV("setting effect to %s...", strEffect);
