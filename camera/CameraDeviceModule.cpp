                                      ...
                                      struct preview_stream_ops* window)
{
    ...
    if (dev->priv == NULL) {
        ALOGW_IF(window, "camera device already released and, "
...
static int cam_dev_close(struct hw_device_t* device)
{
    ...
    if (device != (hw_device_t*)g_dev) {
        ALOGE("%s: got device %p. expect %p!", __func__, device, g_dev);
        return -EINVAL;
    }

    if (g_dev == NULL) {
        ALOGE("%s: Camera deivce already closed. Do nothing!", __func__);
        goto exit;
    }
...
static camera_device_t* cam_dev_get_instance(int id)
{
    ...
    if (g_dev == NULL) {
        ALOGE("%s: Fail to alloc mem for cam_dev_t!", __func__);
        goto done;
    }
    ...
    if (g_dev->priv == NULL) {
        ALOGE("%s: Fail to alloc mem for CameraHardware!", __func__);
