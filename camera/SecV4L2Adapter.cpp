...
int SecV4L2Adapter::nPixfmt(const char* strPixfmt)
{
    ...
    if (NULL == strPixfmt) {
        ALOGE("%s: given NULL for strPixfmt!", __func__);
        return -1;
    }
    ...
    ALOGV("%s: converted %s to %d", __func__, strPixfmt, v4l2Pixfmt);
...
int SecV4L2Adapter::enumSceneMode(const char* strScene)
{
    ...
    if (NULL == strScene) {
        ALOGE("%s: given NULL for strScene!", __func__);
        return -1;
    }
    ...
    ALOGV("%s: converted %s to %d", __func__, strScene, enumScene);
    return enumScene;
}
...
int SecV4L2Adapter::enumFlashMode(const char* strFlash)
{
    ...
    if (NULL == strFlash) {
        ALOGE("%s: given NULL for strFlash!", __func__);
        return -1;
    }
    ...
    ALOGV("%s: converted %s to %d", __func__, strFlash, enumFlash);
    return enumFlash;
}
...
int SecV4L2Adapter::enumBrightness(int ev)
{
    ...
    if (0 > ev_enum || EV_MAX < ev_enum) {
        ALOGE("%s: failed to convert given ev, %d to enum. rollback to default.",
              __func__, ev);
        ev_enum = EV_DEFAULT;
    }

    ALOGV("%s: converted %d to %d", __func__, ev, ev_enum);
    return ev_enum;
}
...
int SecV4L2Adapter::enumWB(const char* strWB)
{
    ...
    if (NULL == strWB) {
        ALOGE("%s: given NULL for strWB!", __func__);
        return -1;
    }
    ...
    ALOGE_IF(0 > enumWB, "%s, %s unsupported!", __func__, strWB);
    ALOGV("%s: converted %s to %d", __func__, strWB, enumWB);
...
int SecV4L2Adapter::enumEffect(const char* strEffect)
{
    ...
    if (NULL == strEffect) {
        ALOGE("%s: given NULL for strEffect!", __func__);
        return -1;
    }
    ...
    ALOGE_IF(0 > enumEffect, "%s, %s unsupported!", __func__, strEffect);
    ALOGV("%s: converted %s to %d", __func__, strEffect, enumEffect);
...
int SecV4L2Adapter::enumFocusMode(const char* strFocus)
{
    ...
    if (NULL == strFocus) {
        ALOGE("%s: given NULL for strFocus!", __func__);
        return -1;
    }
    ...
    ALOGE_IF(0 > enumFocus, "%s, %s unsupported!", __func__, strFocus);
    ALOGV("%s: converted %s to %d", __func__, strFocus, enumFocus);
...
{
    ALOGW_IF(_bufSize == 0, "Actual buffer size not available yet!");
...
SecV4L2Adapter::SecV4L2Adapter(const char* path, int ch):
    ...
    _bufSize(0)
{
    ALOGI("opening %s (ch=%d)...", path, ch);
    int err = 0;
    err |= _openCamera(path);
    err |= _setInputChann(ch);

    if (err) {
        ALOGE("!! V4L2 init failed !! path=%s, ch=%d", path, ch);
...
int SecV4L2Adapter::_openCamera(const char* path)
{
    ...
    if (_fd) {
        ALOGW("fd(%d) already opened. close it first!", _fd);
        close(_fd);
    }
...
int SecV4L2Adapter::_setInputChann(int ch)
{
    ...
    if (_fd == 0) {
        ALOGE("%s: camera not opened!", __func__);
        return -1;
    }

    ALOGI("%s: enum chan", __func__);
    input.index = ch;
    ...
    ALOGI("Name of input channel[%d] is %d\n", input.index, _fd);
...
int SecV4L2Adapter::_setFmt(int w, int h, unsigned int fmt, int flag)
{
    ...
    while (ioctl(_fd, VIDIOC_ENUM_FMT, &fmtdesc) == 0) {
        if (fmtdesc.pixelformat == fmt) {
            ALOGI("passed fmt = %s found pixel format[%d]: %s\n",
    ...
    if (!found) {
        ALOGE("unsupported pixel format, %d\n", fmt);
        return -1;
    }
...
int SecV4L2Adapter::_reqBufs(int n)
{
    ...
    ALOGW_IF(n != 1 && req.count == 1, "insufficient buffer avaiable!");
...
int SecV4L2Adapter::_queryBuf(int idx, int* length, int* offset)
{
    ...
    ALOGV("buffer-%d: length = %d, offset = %d", idx, l, o);

    if (_bufSize == 0)
        _bufSize = l;

    ALOGE_IF(_bufSize != (size_t)l,
                              ...
                              int flag)
{
    ...
    ALOGW_IF(n > _bufCnt, "only %d buffers available! req = %d", _bufCnt, n);
...
int SecV4L2Adapter::mapBuf(int idx)
{
    ...
    if (err || length == 0) {
        ALOGE("%s: aborted mmap-%d!", __func__, idx);
        return -1;
    }
    ...
    void* start = mmap(0, length, PROT_READ | PROT_WRITE, MAP_SHARED,
                       _fd, offset);
    if (start == MAP_FAILED) {
        ALOGE("%s: mmap() failed. idx = %d, length = %d, offset = %d",
              __func__, idx, length, offset);
        return -1;
    }

    if (_bufMapStart[idx] != NULL) {
        ALOGE("%s: mmaped alread in %d!", __func__, idx);
        return -1;
    }
...
int SecV4L2Adapter::closeBufs()
{
    ...
    for (unsigned int i = 0; i < _bufCnt; i++) {
        ...
        ALOGV("munmap-%d : addr = 0x%p size = %d\n",
...
int SecV4L2Adapter::setScenarioMode(int width, int height, int fmt, v4l2_field field)
{
    ...
    ret = ioctl(_fd, VIDIOC_S_FMT, &format);
	if (ret < 0) {
		ALOGE("ERR(%s): ioctl failed", __func__);
		return ret;
	}
...
int SecV4L2Adapter::startStream(bool on)
{
    ...
    if (ret < 0) {
        ALOGE("ERR(%s): Failed stream %s", __func__,
              on ? "on" : "off");
        return ret;
    }
...
int SecV4L2Adapter::qBuf(unsigned int idx)
{
    ...
    if (idx > (_bufCnt - 1)) {
        ALOGE("%s: invalid index, %d!", __func__, idx);
        return -1;
    }
...
int SecV4L2Adapter::dqBuf(void)
{
    ...
    if (ret < 0) {
        ALOGE("%s: VIDIOC_DQBUF failed\n", __func__);
        return ret;
    }

    if (v4l2_buf.index > (_bufCnt - 1)) {
        ALOGE("%s: invalid index, %d!", __func__, v4l2_buf.index);
        return -1;
    }
...
int SecV4L2Adapter::blk_dqbuf(void)
{
    ...
    ret = ioctl(_fd, VIDIOC_DQBUF, &v4l2_buf);
    if (ret < 0) {
        // ALOGV("VIDIOC_DQBUF first is empty") ;
        waitFrame();
        return dqBuf();
    } else {
        ...
        while (ret == 0) {
            ret = ioctl(_fd, VIDIOC_DQBUF, &v4l2_buf);
            if (ret == 0) {
                ALOGV("VIDIOC_DQBUF is not still empty %d", v4l2_buf.index);
                qBuf(index);
                index = v4l2_buf.index;
            } else {
                ALOGV("VIDIOC_DQBUF is empty now %d ",
                      index);
                return index;
            }
        }
...
int SecV4L2Adapter::getParm(struct sec_cam_parm* parm)
{
    ...
    if (parm == NULL) {
        ALOGE("%s: given NULL for sec_cam_parm *!!", __func__);
        return -1;
    }
...
int SecV4L2Adapter::waitFrame(int timeout)
{
    ...
    if (ret < 0) {
        ALOGE("ERR(%s):poll error, revent = %d\n", __func__,
              _poll.revents);
        return ret;
    }

    if (ret == 0) {
        ALOGE("ERR(%s):No data in %d msecs..\n", __func__, timeout);
        return ret;
    }
