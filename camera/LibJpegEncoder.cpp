...
int LibJpegEncoder::doCompress(EncoderParams* params, uint8_t* inBuff, int inBuffSize)
{
    ...
    ALOGV("jpeg compressed. _jpegSize = %d", _jpegSize);
...
{
    ALOGE_IF(_outBuff == NULL || _jpegSize == 0,
...
int LibJpegEncoder::copyOutput(uint8_t* outBuff, int outBuffSize, bool skipSOI)
{
    if (_outBuff == NULL || _jpegSize == 0) {
        ALOGE("%s: seems JPEG not readied. yet?", __func__);
        return 0;
    }

    if (outBuffSize < _jpegSize) {
        ALOGE("%s: too small buffer for JPEG output. outBuffSize = %d, _jpegSize = %d",
...
bool LibJpegEncoder::_checkParamsValid(EncoderParams* params)
{
    if (params->width < 2 || params->height < 2) {
        ALOGE("%s: Not support too small input %dx%d!", __func__,
              params->width, params->height);
        return false;
    }

    if (params->quality < 1) {
        ALOGE("%s: Not support too small quality, %d!", __func__,
              params->quality);
        return false;
    }

    if (params->format != V4L2_PIX_FMT_YUYV) {
        ALOGE("%s: Not supported format, %d", __func__,
              params->format);
        return false;
    }
...
{
    if (_outBuff) {
        ALOGV("%s: freeing _outBuff, %dbytes", __func__, _outBuffSize);
        free(_outBuff);
        _outBuff = NULL;
    }
    _outBuffSize = 0;
...
void LibJpegEncoder::_initOutBuff(EncoderParams* params)
{
    ...
    if (buffSize == 0) {
        ALOGE("%s: input size not determined. yet?", __func__);
        return;
    }

    if (_outBuff && _outBuffSize < buffSize) {
        ALOGV("%s: will realloc _outBuff for %dbytes", __func__, buffSize);
        _deinitOutBuff();
    }

    if (_outBuff == NULL) {
        ALOGV("%s: alloc %dbytes for jpeg out", __func__, buffSize);
