...
#define LOG_CAMERA_FUNC_ENTER  ALOGV("enter %s", __func__)
#define LOG_CAMERA_FUNC_EXIT   ALOGV("exit %s", __func__)
...
#define CHECK(T)                                \
    if (!(T)) {                                 \
        ALOGE("%s+%d: Expected %s but failed!!", \
                __func__, __LINE__, #T);        \
    }

#define CHECK_EQ(A, B)                          \
    if (!(A == B)) {                            \
        ALOGE("%s+%d: Expected %s(%d) == %s(%d)!!", \
