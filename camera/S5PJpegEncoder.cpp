...
{
    if (_fd > 0) {
        ALOGE_IF(api_jpeg_encode_deinit(_fd) != JPEG_OK,
...
int S5PJpegEncoder::setImgFormat(int w, int h, int f, int q)
{
    ...
    if ((unsigned int)w == _params.width
            ...
            && sf == _params.out_fmt) {
        ALOGV("%s: same as before. do nothing!", __func__);
        return 0;
    }
    ...
    ALOGI("Set Params. image size=%dx%d, quality=%d", w, h, q);
...
int S5PJpegEncoder::doCompress(uint8_t* inBuff, int inBuffSize)
{
    ...
    if (_fd == 0) {
        ALOGE("%s: Jpeg device not opened!", __func__);
        return -1;
    }
    ...
    if (ret != JPEG_ENCODE_OK) {
        ALOGE("%s: Failed encode to JPEG!", __func__);
        return -1;
    }

    ALOGI("JPEG encoded. imagesize=%dx%d datasize=%d",
...
int S5PJpegEncoder::copyOutput(uint8_t* outBuff, int outBuffSize, bool skipSOI)
{
    ...
    if (0 >= outSize) {
        ALOGE("%s: seem no output readied!", __func__);
        return -1;
    }

    ALOGW_IF(outBuffSize != outSize,
...
int S5PJpegEncoder::_reset(void)
{
    ...
    if (_fd > 0) {
        ...
        if (ret != JPEG_OK) {
            ALOGE("ERR(%s):Fail on api_jpeg_encode_deinit\n", __func__);
            _fd = 0;
            return -1;
        }
    }
...
jpeg_stream_format S5PJpegEncoder::_getOutFormat(int inFmt)
{
    ...
    defalut:
        ALOGE("%s: S5PJpegEncoder not support input format, %d", __func__, inFmt);
        break;
    }

    ALOGV("%s: inFmt=%d, outFmt=%d", __func__, inFmt, outFmt);
...
jpeg_img_quality_level S5PJpegEncoder::_getQualityEnum(int quality)
{
    ...
    if (quality > 100 || quality < 0) {
        ALOGE("%s: quality should value in 0~100. given %d.", __func__, quality);
    ...
    ALOGV("%s: quality=%d, enumQuality=%d", __func__, quality, enumQuality);
